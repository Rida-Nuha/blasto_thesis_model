"""
Multi-Task DINOv2 Baseline
Self-Supervised Vision Transformer Comparison Architecture for Comprehensive Embryo Grading
"""
...
SAVE_DIR = "/kaggle/working/saved_models/dinov2_champion_baseline"        
...
train_transform = transforms.Compose([
    ...
    transforms.RandomCrop(224), # 224 is perfectly divisible by 14 (DINOv2 patch size)
...
# ============================================================
# 3. MULTI-TASK DINOv2 MODEL
# ============================================================
class MultiTaskDINOv2(nn.Module):
    def __init__(self, dropout_rate=0.3):
        super().__init__()
        
        print("Loading Foundational DINOv2 (ViT-Small) Weights via Torch Hub...")
        # Pulls the official dinov2_vits14 model (384 embedding dimension)
        self.backbone = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14', pretrained=True)
        
        # DINOv2-Small outputs a 384-dimensional feature vector
        in_features = 384
        
        # Exact same head architecture as your Swin-T to ensure fair comparison
    ...
    def forward(self, x):
        # DINOv2 naturally returns the CLS token feature vector when called
...
def train_single_model(seed, train_loader, val_loader, w_exp, w_icm, w_te):
    print(f"\n{'='*70}")
    print(f"TRAINING DINOv2 BASELINE (SEED {seed})")
    print(f"{'='*70}\n")
    
    set_seed(seed)
    model = MultiTaskDINOv2(DROPOUT_RATE).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_acc > best_avg_acc:
            ...
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"dinov2_multitask_seed{seed}_best.pth"))
