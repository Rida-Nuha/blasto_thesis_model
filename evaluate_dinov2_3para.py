"""
Multi-Task DINOv2 Baseline Evaluation Script
Standard Categorical Outputs with MC Dropout Uncertainty Analysis
Self-Supervised Foundational Model Baseline for Thesis Comparison
"""
...
# 🚨 Points to the DINOv2 saved model
MODEL_PATH = "/kaggle/working/saved_models/dinov2_champion_baseline/dinov2_multitask_seed42_best.pth"
SAVE_DIR = "/kaggle/working/evaluation_plots_dinov2_baseline"
...
# ============================================================
# 3. DINOv2 ARCHITECTURE WITH DROPOUT
# ============================================================
class MultiTaskDINOv2WithUncertainty(nn.Module):
    def __init__(self, dropout_rate=0.3):
        super().__init__()
        # Load the architecture without downloading pretrained weights again
...
def plot_uncertainty_boxplots(y_true, y_pred, uncertainties, title, filename):
    ...
    plt.title(f"{title} - Uncertainty Distribution (DINOv2)")
...
def evaluate_mc_dropout():
    ...
    print("Loading DINOv2 Baseline Model for Evaluation...")
    model = MultiTaskDINOv2WithUncertainty(dropout_rate=DROPOUT_RATE).to(DEVICE)
    ...
    print("\n==================================================")
    print("FINAL CLINICAL METRICS (DINOv2 BASELINE)")
    print("==================================================")
    print("\nEXPANSION RESULTS:")
    ...
    plot_confusion_matrix(all_exp_true, all_exp_pred, ['1', '2', '3', '4', '5'], 'DINOv2 Baseline - Expansion', 'cm_exp_dinov2.png', labels=[1, 2, 3, 4, 5])
    plot_confusion_matrix(all_icm_true, all_icm_pred, ['A', 'B', 'C'], 'DINOv2 Baseline - ICM', 'cm_icm_dinov2.png', labels=[0, 1, 2])
    plot_confusion_matrix(all_te_true, all_te_pred, ['A', 'B', 'C'], 'DINOv2 Baseline - TE', 'cm_te_dinov2.png', labels=[0, 1, 2])
    
    plot_uncertainty_boxplots(all_exp_true, all_exp_pred, all_exp_ent, 'Expansion', 'uncertainty_exp_dinov2.png')
    plot_uncertainty_boxplots(all_icm_true, all_icm_pred, all_icm_ent, 'ICM', 'uncertainty_icm_dinov2.png')
    plot_uncertainty_boxplots(all_te_true, all_te_pred, all_te_ent, 'TE', 'uncertainty_te_dinov2.png')
