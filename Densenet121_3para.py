"""
Multi-Task DenseNet121 Baseline
Medical CNN Comparison Architecture for Comprehensive Embryo Grading (EXP, ICM, TE)
"""
...
SAVE_DIR = "/kaggle/working/saved_models/densenet121_champion_baseline"        
...
# ============================================================
# 3. MULTI-TASK DENSENET121 MODEL
# ============================================================
class MultiTaskDenseNet121(nn.Module):
    def __init__(self, dropout_rate=0.3):
        ...
        print("Injecting Standard ImageNet Weights for DenseNet121...")
        self.backbone = densenet121(weights=DenseNet121_Weights.IMAGENET1K_V1)
        
        # In DenseNet, the final layer is called 'classifier'
...
def train_single_model(seed, train_loader, val_loader, w_exp, w_icm, w_te):
    print(f"\n{'='*70}")
    print(f"TRAINING DENSENET121 BASELINE (SEED {seed})")
    print(f"{'='*70}\n")
    
    set_seed(seed)
    model = MultiTaskDenseNet121(DROPOUT_RATE).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_acc > best_avg_acc:
            ...
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"densenet121_multitask_seed{seed}_best.pth"))
