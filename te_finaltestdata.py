"""
TE TEST SET EVALUATION
Evaluates 5 trained TE models on Gardner_test_gold.xlsx
Same structure as ICM test code
"""
...
# Filter valid TE_gold values
test_df_clean = test_df[test_df['TE_gold'].notna()].copy()
test_df_clean = test_df_clean[test_df_clean['TE_gold'] != 'ND'].copy()
test_df_clean = test_df_clean[test_df_clean['TE_gold'] != 'NA'].copy()

# Convert to numeric
test_df_clean['TE_gold'] = pd.to_numeric(test_df_clean['TE_gold'], errors='coerce')
test_df_clean = test_df_clean[test_df_clean['TE_gold'].notna()].copy()

print(f"\nValid TE samples: {len(test_df_clean)}")
print(f"\nTE_gold distribution:")
print(test_df_clean['TE_gold'].value_counts().sort_index())

# Binary conversion (same as training: TE >= 2 is Good)
test_df_clean['label'] = test_df_clean['TE_gold'].apply(lambda x: 1 if x >= 2 else 0)

print(f"\nBinary label distribution:")
poor_count = (test_df_clean['label']==0).sum()
good_count = (test_df_clean['label']==1).sum()
print(f"  Poor (TE < 2): {poor_count} ({poor_count/len(test_df_clean)*100:.1f}%)")
print(f"  Good (TE >= 2): {good_count} ({good_count/len(test_df_clean)*100:.1f}%)")
...
# ============================================================
print("\n" + "="*70)
print("LOADING TRAINED TE MODELS")
print("="*70)
...
for seed in SEEDS:
    ...
    model_path = f"{MODEL_DIR}/TE_silver_seed{seed}_best.pth"
...
# ============================================================
print("\n" + "="*70)
print("FINAL TEST SET RESULTS - TE")
print("="*70)
...
paper_te = 56.33  # From the paper
improvement = (test_acc * 100) - paper_te

print(f"\nPaper (2025) - TE: {paper_te:.2f}%")
print(f"Your Model   - TE: {test_acc*100:.2f}%")
print(f"Improvement:       {improvement:+.2f} percentage points")
print(f"Relative Gain:     {(improvement/paper_te)*100:+.2f}%")
...
print("\n" + "="*70)
if good_recall > 0.80 and poor_recall > 0.80 and test_acc > 0.85:
    ...
    if good_recall < 0.50 or poor_recall < 0.50:
        print("   ⚠️ One class has low recall - check for imbalance")
print("="*70)

print("\n✅ TE TEST EVALUATION COMPLETE!")
...
results_df = pd.DataFrame({
    ...
    'TE_gold': test_df_clean['TE_gold'].values,
...
results_df.to_csv('TE_test_predictions.csv', index=False)
print(f"\n✅ Predictions saved to: TE_test_predictions.csv")
