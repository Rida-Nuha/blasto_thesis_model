...
4. Longer training
5. FIXED: Better LR, Mixup, OneCycleLR
"""
...
STAGE1_EPOCHS = 25  # Slightly longer stage 1
STAGE2_EPOCHS = 40  # Reduced from 80
STAGE1_LR = 2e-3  # Increased from 1e-3
STAGE2_LR = 5e-5  # Increased from 1e-5 (5x higher!)
WEIGHT_DECAY = 5e-4  # Stronger regularization
TRAIN_SPLIT = 0.85
NUM_WORKERS = 2  # Fixed for Kaggle
PATIENCE = 15  # Reduced - stop earlier if not improving
SEED = 42
...
def main():
    ...
    optimizer = optim.AdamW([
        {'params': model.backbone.features[0:2].parameters(), 'lr': STAGE2_LR * 0.1},  # Early layers
        {'params': model.backbone.features[2:4].parameters(), 'lr': STAGE2_LR * 0.3},  # Mid layers
        {'params': model.backbone.features[4:].parameters(), 'lr': STAGE2_LR * 0.5},   # Late layers
        {'params': model.backbone.norm.parameters(), 'lr': STAGE2_LR * 2},
        {'params': model.backbone.head.parameters(), 'lr': STAGE2_LR * 5}  # Head needs higher LR
    ], weight_decay=WEIGHT_DECAY)
    ...
    if best_val_acc >= 80:
        ...
        print(f"\n⚠️ Consider additional improvements")
